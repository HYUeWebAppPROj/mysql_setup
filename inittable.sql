...
create table if not exists course_storage(stor_sess varchar(255),course_id int(10),course_page int(10),filemime varchar(30) not null,content text(16384)  ,primary key(stor_sess,course_id,course_page),check(course_page>=0),foreign key(stor_sess) references userlist(storage_session_id) on delete cascade on update cascade,foreign key(course_id) references course_list(course_id) on delete cascade on update cascade);
