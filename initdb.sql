...
use mysql;
CREATE USER IF NOT EXISTS 'webapp'@'localhost' IDENTIFIED BY 'hyuwebapp2016~2017~~~'; 
#공동 관리자 계정
CREATE USER IF NOT EXISTS 'webappro'@'localhost' IDENTIFIED BY 'waro';
 #읽기전용계정
CREATE USER IF NOT EXISTS 'webappuser'@'localhost' IDENTIFIED BY 'warw'; 
#사용자가요청하는작업을하는계정
...
grant SELECT, SHOW VIEW, INDEX privileges on waproj.* to 'webappro'@'localhost' IDENTIFIED BY 'waro';
